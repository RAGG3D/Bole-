...
**岗位文件夹状态标记**：submit.py 在每次状态变化时都会自动把 `<岗位文件夹>/STATUS.md`
同步为当前状态（✅ 已提交 / ⏸ 需要补料 / 🧱 阻断转手动 / 📝 手动 / ❓ 未知），用户打开
`Applications/` 即可直接看到每岗投递状态。若发现某岗标记缺失或过期（如台账是旧版本
生成的），运行 `python3 scripts/submit.py mark --job "<岗位文件夹>"` 按台账重建。报告
末尾告知用户这一点。
