...
第四个命令 `/apply`（自动投递）**默认关闭**；开关未开时运行它只会解释开通条件，不碰网络。详见下文「自动投递」一节。

---

## 这是什么

**一句话：一个 Claude Code 技能包（skill repo），不是网站、云服务或独立后端。** 你把仓库 clone 下来，在文件夹里跑 `claude`：Claude 负责采访、理解、打分和写作；仓库里的标准库 Python 脚本负责可重复的抓取、去重、机械分诊、红线扫描、PDF 构建、ATS 路由和投递记账。你的数据全部留在你自己的电脑上。

三层能力一次交付：

- **v0.1**：`/doctor`、`/setup`、`/scan`——建档、发现、打分并生成手动投递包；
- **v0.2**：可选 `/apply`——只向能力地图允许的 ATS 交给 OpenClaw 顺序自动投递；
